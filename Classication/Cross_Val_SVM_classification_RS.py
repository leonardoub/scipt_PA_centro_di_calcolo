...
parameteres = [{'scaler':scalers_to_test, 'red_dim':[PCA()], 'red_dim__n_components':n_features_to_test,'clf__C':[0.001,0.1,10], 'clf__kernel':('poly'), 'clf__degree':[1,2,3,4], 'clf__gamma':[0.1,0.01]},
...
grid = RandomizedSearchCV(pipeline, param_distributions=parameteres, n_iter=30, cv=5, random_state=1, n_jobs=-1, scoring='accuracy')
