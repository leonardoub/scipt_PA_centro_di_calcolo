...
for i in range(1, 21):
    ...
    parameteres = [{'scaler':[RobustScaler()], 'red_dim':[PCA()], 'red_dim__n_components':list(n_features_to_test), 'clf__regressor__C':list(C_range)},
                    {'scaler':[RobustScaler()], 'red_dim':[LinearDiscriminantAnalysis()], 'red_dim__n_components':[2], 'clf__regressor__C':list(C_range)},
                    {'scaler':[RobustScaler()], 'red_dim':[None], 'clf__regressor__C':list(C_range)}]
...
outname = f'best_params_{name}_ ST_regression.csv'
